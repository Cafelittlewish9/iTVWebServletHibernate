...
public class BlackDAOjdbc implements BlackDAO {
	...
	public boolean markBlack(int memberId, int blackedId) {
		...
		int updateCount = 0;
		try (Connection conn = ds.getConnection(); PreparedStatement pstmt = conn.prepareStatement(MARK_BLACK);) {
	...
	public List<BlackVO> getList(int memberId) {
		...
		try (Connection conn = ds.getConnection(); PreparedStatement pstmt = conn.prepareStatement(GET_LIST);) {
			pstmt.setInt(1, memberId);
	...
	public boolean removeBlack(int memberId, int blackedId) {
		...
		try (Connection conn = ds.getConnection(); PreparedStatement pstmt = conn.prepareStatement(REMOVE_BLACK);) {
			pstmt.setInt(1, memberId);
			pstmt.setInt(2, blackedId);
	...
	public boolean removeAll(int memberId) {
		...
		try (Connection conn = ds.getConnection(); PreparedStatement pstmt = conn.prepareStatement(REMOVE_ALL);) {
			pstmt.setInt(1, memberId);
			int updateCount = pstmt.executeUpdate();
			if (updateCount >= 1) {
				removeResult = true;
